...
class SequenceProvider:
    ...
    def generate(self, request, *, repair=None):
...
def test_accepts_valid_json_without_repair(triage_request):
    provider = SequenceProvider(json.dumps(VALID_RESULT))
...
def test_repairs_broken_json_then_accepts_valid_output(triage_request):
    ...
    assert provider.repairs[1].invalid_output == "{"
...
def test_repairs_invalid_enum_then_accepts_valid_mapping(triage_request):
    ...
    provider = SequenceProvider(invalid_result, VALID_RESULT)
    ...
    assert result.urgency == "alta"
    assert any("urgency" in error for error in provider.repairs[1].validation_errors)
...
def test_each_attempt_logs_only_technical_metadata(triage_request):
    provider = SequenceProvider("texto sensible de salida", VALID_RESULT)
    ...
    assert "texto sensible" not in repr(logger.call_args_list)
